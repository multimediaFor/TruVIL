...
class FramesDataset(Dataset):
    def __init__(self, root_dir, n_frames: int, istrain=True):
        ...
        # 获取所有帧和标签的文件名
        self.frame_files = sorted([f for f in os.listdir(self.frame_dir) if os.path.isdir(os.path.join(self.frame_dir, f))])
        self.label_files = sorted([f for f in os.listdir(self.label_dir) if os.path.isfile(os.path.join(self.label_dir, f))])
