...
from AttentionModule import CAF
...
class Encoder(BaseModule):
    ...
    def __init__(
        ...
        # RGB
        self.RGB_patch_embed1 = SpeicalPatchEmbed(
            img_size=img_size,
            patch_size=4,
            in_chans=in_chans,
            embed_dim=embed_dim[0])
        self.RGB_patch_embed2 = PatchEmbed(
            img_size=img_size // 4,
            patch_size=2,
            in_chans=embed_dim[0],
            embed_dim=embed_dim[1])
        self.RGB_patch_embed3 = PatchEmbed(
            img_size=img_size // 8,
            patch_size=2,
            in_chans=embed_dim[1],
            embed_dim=embed_dim[2])
        self.RGB_patch_embed4 = PatchEmbed(
        ...
        self.RGB_norm1 = bn_3d(embed_dim[0])
        self.RGB_norm2 = bn_3d(embed_dim[1])
        self.RGB_norm3 = bn_3d(embed_dim[2])
        self.RGB_norm4 = bn_3d(embed_dim[3])
        ...
        self.Res_patch_embed1 = SpeicalPatchEmbed(
            ...
            in_chans=in_chans,
            embed_dim=embed_dim[0])
        self.Res_patch_embed2 = PatchEmbed(
            img_size=img_size // 4,
            patch_size=2,
            in_chans=embed_dim[0],
            embed_dim=embed_dim[1])
        ...
        self.srm_layer1 = HP3D(in_channels=in_chans)
        self.srm_layer2 = HP3D(in_channels=embed_dim[0])
        self.srm_layer3 = HP3D(in_channels=embed_dim[1])
        self.CAF = CAF(in_channels=embed_dim[2])

    def forward(self, x: torch.Tensor):
        outs = []

        x_res1 = self.srm_layer1(x)
        # ============block1============
        # Res
        x_res = self.Res_patch_embed1(x_res1)
        x_res = self.Res_pos_drop(x_res)
        for blk in self.Res_blocks1:
            x_res = blk(x_res)

        # RGB
        x = self.RGB_patch_embed1(x)
        x = self.RGB_pos_drop(x)
        for blk in self.RGB_blocks1:
            x = blk(x)

        # Fusion 1
        x_res = x_res + self.srm_layer2(x)
        ...
        # ============block2============
        # Res
        x_res = self.Res_patch_embed2(x_res)
        for blk in self.Res_blocks2:
            x_res = blk(x_res)

        # RGB
        x = self.RGB_patch_embed2(x)
        for blk in self.RGB_blocks2:
            x = blk(x)

        # Fusion 2
        x_res = x_res + self.srm_layer3(x)
        x_res = self.Res_norm2(x_res)
        x = self.RGB_norm2(x)
        outs.append(x_res)

        # ============block3============
        # Res
        x_res = self.Res_patch_embed3(x_res)
        for blk in self.Res_blocks3:
            x_res = blk(x_res)
        x_res = self.Res_norm3(x_res)

        # RGB
        x = self.RGB_patch_embed3(x)
        for blk in self.RGB_blocks3:
            x = blk(x)
        x = self.RGB_norm3(x)

        # Fusion 3
        x_res, x = self.CAF(x_res, x)
        outs.append(x_res)

        # ============block4============
        # Res
        x_res = self.Res_patch_embed4(x_res)
        for blk in self.Res_blocks4:
            x_res = blk(x_res)
        x_res = self.Res_norm4(x_res)
        outs.append(x_res)

        # RGB
        x = self.RGB_patch_embed4(x)
        for blk in self.RGB_blocks4:
            x = blk(x)
        x = self.RGB_norm4(x)
        outs.append(x)
        return outs, x_res1